...
def search_song(name):
    d = {"csrf_token": "", "hlpretag": "<span class=\"s-fc7\">", "hlposttag": "</span>", "s": name, "type": "1",
         "offset": "0",
         "total": "true", "limit": "30"}
    d = json.dumps(d)
    rp = get_random()
    param = get_final_param(d, rp)
    result_list = get_music_list(param['params'], param['encSecKey'])
    return result_list, rp
...
if __name__ == '__main__':
    ...
    song_list, random_param = search_song(song_name)
    print('搜索结果如下：')
    if len(song_list) > 0:
